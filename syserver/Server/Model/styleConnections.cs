...
namespace syserver.Shared.Model
{
	...
	{
        public async Task<Style[]> GetCustDetails(string styleNames)
        {
            ...
            try
            {
                using (MySqlConnection conn = mySQLSqlHelper.GetConnection())
                {
                    ...
                    {
                        using (MySqlCommand cmd = new MySqlCommand("sp_StyleGet", conn))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;

                            cmd.Parameters.Add(new MySqlParameter
                            {
                                ParameterName = "@styleNames",
                ...
                using (MySqlConnection conn = mySQLSqlHelper.GetConnection())
                {
                    ...
                    if (conn.State == ConnectionState.Open)
                    {
                        using (MySqlCommand cmd = new MySqlCommand("sp_UpdateStyleData", conn))
                        {
                            ...
                            Console.WriteLine("更新数据：数据库JSON"+json);
